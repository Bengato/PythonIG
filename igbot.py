...
# login method - should already be logged in !!!
    ...
    def logStatus(self, state):
        line = '## '+self.username+' '
        if state == "logged in" or state == "logged out":
            line += state
        else:
            line += "started "+state
...
# accepts wanted hashtag and cycles through posts -> follows posting user
    ...
    def follow_tags(self, hashtag):
        ...
        self.logStatus("following the hashtag "+hashtag)
        for link in links:
...
# accepts wanted hashtag and cycles through posts -> likes posts
    ...
    def like_tags(self, hashtag):
        ...
        for post in posts:
            links.append(post.find_element_by_xpath(
                'a').get_attribute('href'))
        self.logStatus("liking the hastag "+hashtag)
        for link in links:
            self.driver.get(link)
            time.sleep(1)
            self.driver.find_elements_by_xpath(
    ...
    def like_user(self, username):
        ...
        for post in posts:
            links.append(post.find_element_by_xpath(
                'a').get_attribute('href'))
        self.logStatus("liking posts by "+username)
        for link in links:
            self.driver.get(link)
            time.sleep(1)
            self.driver.find_elements_by_xpath(
